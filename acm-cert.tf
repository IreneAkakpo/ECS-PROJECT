...
resource "aws_acm_certificate" "ecs-cert" {
  domain_name               = "techgirlrennie.com"
  subject_alternative_names = ["*.techgirlrennie.com"]
...
resource "aws_route53_record" "route53_record" {
  for_each = {
    for dvo in aws_acm_certificate.ecs-cert.domain_validation_options : dvo.domain_name => {
